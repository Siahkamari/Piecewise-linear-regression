function f_hat = dc_fit_multi_class(y, X, lambda)
% learns a k score functions for classification  given data and regularization constant
...
%% outputs
% learned dc function       a cell containing k functions, f_hat{k} : R ^(n x d) -> R ^(n)  
...
A1 = [A_vivj, sparse(n*(n-1),n)  , A_gxixj, -A_gxixj, sparse(n*(n-1),2*n*d), sparse(n*(n-1),n), sparse(n*(n-1),1)];
A2 = [sparse(n*(n-1),1*n), A_vivj, sparse(n*(n-1),2*n*d), A_gxixj, -A_gxixj, sparse(n*(n-1),n), sparse(n*(n-1),1)];
b1 = sparse(n*(n-1),1);
b2 = sparse(n*(n-1),1);
...
A_empty = sparse(n*(n-1), 2*n + 4*n*d + n + 1);
...
A_empty = sparse(n, 2*n + 4*n*d + n + 1);
...
I = 1:n;
V = -ones(n,1);
J = (1:n)' + (y-1)*(2*n+4*n*d + n + 1);
A4t = sparse(I, J, V, n, k*(2*n + 4*n*d + n + 1)) + ...
    sparse(I, n+ J, -V, n, k*(2*n + 4*n*d + n + 1));

A4 = [Av, -Av, sparse(n,4*n*d), -Av, zeros(n,1)];
b4 = -ones(n,1);
...
%     warning('Gurobi is not installed/working. Instead using MATLAB linear program solvers.')
    ...
    lb = zeros(1, k*(2*n + 4*n*d + n + 1));
    z = linprog(c, [A1_all;A2_all;A3_all;A4_all],[b1;b2;b3;b4], [],[], lb, [], options );
end

f_hat = cell(k,1);
for kk=1:k
    shift = (kk-1) *(2*n + 4*n*d + n + 1);
